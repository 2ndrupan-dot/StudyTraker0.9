...
function ConnectionStatus() {
  ...
  const visible = !online || showBackOnline || syncStatus !== 'idle';
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={cn(
          'fixed top-2 right-2 z-50 flex items-center gap-1 shadow-sm rounded-full px-1.5 py-0.5 text-[10px] font-semibold border',
          cls
        )}
      >
        {spinning ? (
          <motion.div
            ...
            className="w-2 h-2 border border-slate-400 border-t-transparent rounded-full"
          />
        ) : (
...
function InstallSection() {
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Learning Sathi', url });
      } else {
  ...
  return (
    <div className="fixed left-0 top-0 bottom-0 w-64 gradient-sidebar flex flex-col z-30 shadow-xl">
      ...
      <div className="p-3 border-t border-white/10 space-y-2">
        <InstallSection />
        <p className="text-[10px] text-white/30 text-center pt-1">Learning Sathi v2.0</p>
      </div>
    </div>
  );
}
